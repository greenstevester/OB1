...
Embeddings are generated by self-hosted TEI (bge-small), so they're effectively free:

| Library size | Embedding cost | Wall-clock time |
|---|---|---|
| 1,000 highlights | $0 | <1 min |
| 10,000 highlights | $0 | ~5 min |
| 50,000 highlights | $0 | ~20 min |
...
- Every highlight has a 384-dim embedding, so semantic search in Open Brain immediately surfaces them alongside your other thoughts.
...
You didn't export one of the required env vars. All three are required. The script errors before any network call so you don't partially import with bad credentials.
